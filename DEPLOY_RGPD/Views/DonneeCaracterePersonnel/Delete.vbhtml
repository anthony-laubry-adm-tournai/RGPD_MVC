@ModelType ienumerable(of RGPD_MVC.DonneeCaracterePersonnel)
@Code
    ViewData("Title") = "Données à caractère personnel"
End Code

<div>
    <h4>Données à caractère personnel : Suppression d'un élément.</h4>
    <hr />
    ...
    <table class="table">
        ...
        @code
            Dim i As Integer = 0
        End Code
        @For Each item As DonneeCaracterePersonnel In Model
            @<tr>
                @Using (Html.BeginForm("Delete", "DonneeCaracterePersonnel", FormMethod.Post))
