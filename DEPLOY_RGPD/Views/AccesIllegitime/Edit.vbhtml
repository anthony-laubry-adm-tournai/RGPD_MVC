@ModelType RGPD_MVC.AccesIllegitime
@Code
    ViewData("Title") = "Édition : accès illégitime aux données"
End Code

@Using (Html.BeginForm("Edit", "AccesIllegitime", FormMethod.Post, New With {.enctype = "multipart/form-data"}))
    ...
    @<div class="menu-2">
        ...
        <input type="submit" id="AccIll" name="AccIll" value="- Accès illégitime à des données" class="linkButton mnuCreaRisque-current" />
        ...
        @Html.ActionLink("Annuler", "Index", "Traitement", "", htmlAttributes:=New With {.class = "btn btnMenu "})
         <input type="submit" name="Precedent" value="Precedent" class="btn btnMenu" />
             ...
             @If ViewBag.ListeFichier IsNot Nothing Then
                 ...
                 @<div>
                     @If item.Nom.Length > 26 Then
                         ...
                         @Html.ActionLink(nom, "OpenPDF", "AccesIllegitime", New With {.file = item.Path}, htmlAttributes:=New With {.name = "openPDF", .target = "_blank"})
                     Else
                         @Html.ActionLink(item.Nom, "OpenPDF", "AccesIllegitime", New With {.file = item.Path}, htmlAttributes:=New With {.name = "openPDF", .target = "_blank"})
                     End If
                     @*<br />*@
                     @Html.ActionLink("Supprimer", "DeleteDoc", "AccesIllegitime", New With {.id = item.Id, .idTrt = item.IdTraitement}, htmlAttributes:=New With {.class = "boutonSup"})
                 </div>
                 @<br />
             Next
         End If
    ...
    @<div class="form-horizontal">
        <div style="border: 1px solid grey; text-align: center; margin-top: 1%; margin-bottom: 10px; padding-top: 10px; padding-bottom: 10px;">
            ...
            <span style="color: #FD4664; ">Accès illégitime à des données</span><br />
            <span style="font-size:11px;">
                Analysez les causes et conséquences d'un accès illégitime à des données,  <br />
