@ModelType IEnumerable(of RGPD_MVC.LegitimationCaracterePersonnel)
@Code
    ViewData("Title") = "Base de légitimation à caractère personnel"
End Code

<div>
    <h4>Légitimation à caractère personnel : Suppression d'un éklément.</h4>
    <hr />
    ...
    <table class="table">
        ...
        @code
            Dim i As Integer = 0
        End Code
        @For Each item As LegitimationCaracterePersonnel In Model
            @<tr>
                @Using (Html.BeginForm("Delete", "LegitimationCaracterePersonnel", FormMethod.Post))
