@ModelType list(of RGPD_MVC.CauseIncident)
@Code
    ViewData("Title") = "Delete"
End Code

<div>
    <h4>Cause d'incident : Suppression d'un élément.</h4>
    <hr />
    <table class="table">
        <tr>
        ...
        @code
            Dim i As Integer = 0
        End Code
        @For Each item As causeincident In Model
            @<tr>
                @Using (Html.BeginForm("Delete", "CauseIncident", FormMethod.Post))
                    @<td style="display:none;">@Html.Hidden("Id", item.Id)</td>

                    @<td>@Html.EditorFor(Function(modelItem) modelItem(i).Libelle, New With {.htmlAttributes = New With {.class = "form-control", .style = "margin-top:12px;width:100%;min-width:300px;", .readonly = "readonly"}})</td>
