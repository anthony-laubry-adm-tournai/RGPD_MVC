...
<h2>Édition de la déclaration d'un incident</h2>

@Using (Html.BeginForm("Edit", "Incident", FormMethod.Post, New With {.enctype = "multipart/form-data"}))
            ...
            @For Each _type As TypeIncident In lstType
                ...
                @<div class="col-md-6">
                    @If Model.TypeIncident_Incident.Select(Function(x) x.IdTypeIncident).Contains(_type.Id) Then
                        @Html.CheckBox("typeIncident", True, htmlAttributes:=New With {.value = _type.Id, .id = _type.Libelle})
                    Else
        ...
        <div id="DescrTypeIncident" class="row">
            ...
            <div class="col-md-6">
                @If Not String.IsNullOrEmpty(txtAutre) Then
                    @Html.TextArea("TxtAutre",txtAutre, htmlAttributes:=New With {.class = "form-control", .style = "margin-left:-55px;max-width:393px;width:100%;min-height:150px;height:100%;"})
                Else
            ...
            <div class="row">
                ...
                <div class="col-md-10">
                    ...
                    @If ViewBag.CheminFichier IsNot Nothing Then
                            @<div>@Html.ActionLink(ViewBag.CheminFichier.Nom, "Download", "Incident", New With {.param = Server.MapPath(ViewBag.CheminFichier.Path)}, htmlAttributes:=New With {.target = "_blank"})</div>
                    End If
                </div>
            </div>
        </div>
